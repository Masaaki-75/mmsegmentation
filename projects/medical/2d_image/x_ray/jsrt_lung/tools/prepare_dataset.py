...
src_img_train_dir = os.path.join(
    root_path, 'JSRT/segmentation01/Segmentation01/train/org')
src_mask_train_dir = os.path.join(
    root_path, 'JSRT/segmentation01/Segmentation01/train/label')
src_img_test_dir = os.path.join(root_path,
                                'JSRT/segmentation01/Segmentation01/test/org')
src_mask_test_dir = os.path.join(
    root_path, 'JSRT/segmentation01/Segmentation01/test/label')
...
for i, src_mask_dir in enumerate((src_mask_train_dir, src_mask_test_dir)):
    ...
    for path, name in zip(mask_paths, mask_names):
        ...
        mask = convert_label(mask, convert_dict={0: 0, 255: 1})
